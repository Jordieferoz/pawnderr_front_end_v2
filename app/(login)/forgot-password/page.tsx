...
export default function ForgotPassword() {
    ...
    const {
        ...
        formState: { errors: phoneErrors, isValid: isPhoneValid, isSubmitting: isPhoneSubmitting }
    ...
    const {
        ...
        formState: { errors: resetErrors, isValid: isResetValid, isSubmitting: isResetSubmitting },
        watch: watchReset
    ...
    const onPhoneSubmit = async (data: ForgotPasswordValues) => {
        try {
            ...
            const response = await forgotPassword({ phone: formattedPhone });
            ...
            showToast({
                type: "error",
                message: error?.response?.data?.message || "Failed to send OTP. Please check the number."
            });
        }
    };
        ...
        <div className="relative min-h-[100dvh] px-5 py-6 md:px-8">
            ...
            <div className="sm:mx-auto sm:w-full sm:max-w-[766px] md:bg-white md:shadow-[0px_4px_16.4px_0px_#0000001A] rounded-4xl md:px-20 md:py-7 relative z-20">
                ...
                <div className="md:px-30">
                    ...
                    {step === 1 ? (
                        <form onSubmit={handlePhoneSubmit(onPhoneSubmit)} className="flex flex-col gap-6" noValidate>
                            ...
                            <div className="fixed bottom-0 md:relative py-5 md:pb-0 w-full bg-white shadow-[0px_-4px_12.8px_-3px_#00000012] md:shadow-none">
                                ...
                                <div className="text-center">
                                    <Link href="/sign-in" className="text-primary-theme paragraph1_bold">
                                        Back to Login
                                    </Link>
                                </div>
                            </div>
                        ...
                        <form onSubmit={handleResetSubmit(onResetSubmit)} className="flex flex-col gap-6" noValidate>
                            <div>
                                <label className="mb-2 block text-sm font-medium text-gray-900">Enter OTP</label>
                                <Controller
                                    control={resetControl}
                                    name="otp"
                                    render={({ field }) => (
                            ...
                            <div className="relative">
                                ...
                                {resetErrors.newPassword && (
                                    <p className="mt-1 text-sm text-red-500">{resetErrors.newPassword.message}</p>
                                )}
                            </div>
